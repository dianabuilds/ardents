...
func (s *SeedManager) Create(seedPassword string) (mnemonic string, keys *DerivedKeys, err error) {
	if strings.TrimSpace(seedPassword) == "" {
	...
	return s.Import(mnemonic, seedPassword)
}

func (s *SeedManager) Import(mnemonic, seedPassword string) (normalizedMnemonic string, keys *DerivedKeys, err error) {
	...
	env, err := EncryptSeed([]byte(mnemonic), []byte(seedPassword))
	if err != nil {
		return "", nil, err
	}
...
func (s *SeedManager) Export(seedPassword string) (string, error) {
	if strings.TrimSpace(seedPassword) == "" {
	...
	plaintext, err := DecryptSeed(env, []byte(seedPassword))
	if err != nil {
		s.mu.Lock()
		defer s.mu.Unlock()
...
func (s *SeedManager) ChangePassword(oldSeedPassword, newSeedPassword string) error {
	oldSeedPassword = strings.TrimSpace(oldSeedPassword)
	newSeedPassword = strings.TrimSpace(newSeedPassword)
	if oldSeedPassword == "" || newSeedPassword == "" {
		return ErrPasswordRequired
	}
	...
	mnemonicBytes, err := DecryptSeed(env, []byte(oldSeedPassword))
	if err != nil {
		s.mu.Lock()
		defer s.mu.Unlock()
		s.onFailedPasswordAttempt()
	...
	newEnv, err := EncryptSeed(mnemonicBytes, []byte(newSeedPassword))
	if err != nil {
		return err
	}
