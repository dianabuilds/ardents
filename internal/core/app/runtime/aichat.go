...
func (r *Runtime) buildAITranscript(taskID string, input aichat.Input) (string, error) {
	transcript := buildAITranscriptBody(taskID, input)
	if input.Policy.Visibility == "encrypted" {
		...
		return r.storeAITranscriptNode(node)
	}
	n := contentnode.Node{
		V:           1,
	...
	return r.storeAITranscriptNode(n)
}

func buildAITranscriptBody(taskID string, input aichat.Input) map[string]any {
...
func (r *Runtime) storeAITranscriptNode(node contentnode.Node) (string, error) {
