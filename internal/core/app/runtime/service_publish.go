...
func (r *Runtime) publishServiceHeadAndLeaseSet(info localServiceInfo) error {
	...
	leaseExpires := nowMs + leaseTTL
	if leaseExpires-nowMs > maxTTL {
		leaseExpires = nowMs + maxTTL
	}
	...
	keypair, err := lockeys.LoadOrCreate(dirs.LKeysDir(), info.ServiceID)
	if err != nil {
		return err
	}
	...
	headExpires := nowMs + 3_600_000
	if headExpires-nowMs > maxTTL {
		headExpires = nowMs + maxTTL
	}
	...
	if status, code := r.netdb.Store(leaseBytes, nowMs); status != "OK" {
	...
	if status, code := r.netdb.Store(headBytes, nowMs); status != "OK" {
		return errors.New(code)
	}
...
func leasesEqual(a []netdb.Lease, b []netdb.Lease) bool {
	...
	sort.Slice(aa, func(i, j int) bool {
		if aa[i].GatewayPeerID == aa[j].GatewayPeerID {
			if aa[i].ExpiresAtMs == aa[j].ExpiresAtMs {
				return string(aa[i].TunnelID) < string(aa[j].TunnelID)
			}
			return aa[i].ExpiresAtMs < aa[j].ExpiresAtMs
		}
		return aa[i].GatewayPeerID < aa[j].GatewayPeerID
	})
	sort.Slice(bb, func(i, j int) bool {
		if bb[i].GatewayPeerID == bb[j].GatewayPeerID {
			if bb[i].ExpiresAtMs == bb[j].ExpiresAtMs {
				return string(bb[i].TunnelID) < string(bb[j].TunnelID)
			}
			return bb[i].ExpiresAtMs < bb[j].ExpiresAtMs
		}
		return bb[i].GatewayPeerID < bb[j].GatewayPeerID
	})
	for i := range aa {
