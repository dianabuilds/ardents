...
func TestMessageRequestStateStoreBootstrapDefaultsWhenMissing(t *testing.T) {
	path := filepath.Join(t.TempDir(), "requests.enc")
	store := &MessageRequestStateStore{}
...
func TestMessageRequestStateStorePersistAndReload(t *testing.T) {
	path := filepath.Join(t.TempDir(), "requests.enc")
	store := &MessageRequestStateStore{}
	store.Configure(path, "test-secret")
	...
	reload := &MessageRequestStateStore{}
...
func TestMessageRequestStateStoreBootstrapIOError(t *testing.T) {
	dir := t.TempDir()
	store := &MessageRequestStateStore{}
...
func TestMessageRequestStateStorePersistCreatesPrivateDir(t *testing.T) {
	baseDir := t.TempDir()
