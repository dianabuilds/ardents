...
4) Handshake hints (`router_info` / `router_infos` из handshake) — только как `handshake_hint` и только с коротким TTL.  
   1) Каждая подсказка **ДОЛЖНА** быть отдельной записью `router.info.v1` (SPEC-510) в canonical CBOR, включая подпись.  
   2) Клиент **ДОЛЖЕН** валидировать подпись и TTL записи перед использованием.  
   3) Клиент **МОЖЕТ** принимать через handshake hints записи про третьих лиц (другие `peer_id`), но **ДОЛЖЕН** считать их `untrusted` и хранить только в discovery cache (не в Address Book).  
   4) Клиент **ДОЛЖЕН** ограничивать объём и частоту принятия подсказок: максимум 16 записей `router.info` на один `hello`, и максимум 16 KiB на одну запись.  
   5) Источник `handshake_hint` **НЕ ДОЛЖЕН** заменять trust‑политику и **НЕ ДОЛЖЕН** автоматически создавать алиасы/домены.
