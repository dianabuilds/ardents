...
3. Address Book (SPEC-120) остаётся локальным источником доверия и может “пинить” конкретные peer/service identities, но **НЕ является** механизмом глобального discovery.
...
* `capabilities_digest` (bytes, SHA-256) — хэш списка capabilities, которые peer может объявлять (см. SPEC-300)
...
* если `router_info` присутствует, получатель **ДОЛЖЕН** валидировать его по SPEC-510 и может использовать как “ускорение” bootstrap (без ожидания DHT репликации).
