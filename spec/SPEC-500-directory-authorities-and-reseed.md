...
**Зависимости:** SPEC-001, SPEC-002, SPEC-010, SPEC-110, SPEC-120, SPEC-420  
...
2. Узел/клиент **ДОЛЖЕН** иметь pinned публичные ключи (identity_id) всех 5 DA (в дистрибутиве) либо получить их из доверенного локального источника (Address Book, `trust=trusted`).
...
### 2.2 Overlay reseed (после входа, по явному включению)

После успешного входа в сеть узел **ДОЛЖЕН** выполнять overlay‑reseed только если этот механизм явно включён локальной конфигурацией.
По умолчанию overlay‑reseed выключен. Этот механизм не заменяет внешний reseed “нулевого дня”.

**Ограничение:** любые внешние каталоги и overlay‑сервисы (включая directory‑сервисы DA) **ДОЛЖНЫ** использоваться только если явно включены в локальной конфигурации и проходят локальную trust‑политику.
