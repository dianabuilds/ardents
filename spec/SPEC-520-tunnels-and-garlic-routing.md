...
1. В privacy-first профиле любые прикладные сообщения (`chat.msg.*`, `task.*`) **ДОЛЖНЫ** доставляться через туннели.
2. Длина туннеля по умолчанию: `hop_count_default=3` (SPEC-500). Конфигурация может менять `hop_count` в диапазоне `[2..5]`.
