...
**Статус:** Draft (2026-02-02). Продукт один; версии/профили протокола сосуществуют и не являются отдельными фазами.
...
3. **Приватность и анонимность по умолчанию:** туннели, directory/reseed, NetDB, минимизация метаданных.
...
Профили протокола — это **wire/transport‑варианты**, а не «фазы продукта». Один продукт может поддерживать несколько профилей одновременно.

1. **Direct mode (compat)**  
   * discovery: только config/address book (SPEC-110, раздел 1)
   * доставка: `envelope.v1` напрямую/через простые relays (SPEC-130)
...
### 5.1 Плановые границы (не runtime‑режим)

* MVP‑1/2/3 — это **этапы разработки**, а не отдельные фазы продукта или конфигурации.
* В коде всегда работает **актуальная** реализация; «phase»‑переключателей быть не должно.

### 5.2 Проектные ограничения (зафиксировано)
...
* Целевые платформы: **Windows + Linux**, архитектура **amd64**.
* `legacy/` — reference-only, не часть реализации v1 и допускается удаление при исчезновении нужды.
* Релизы: `CHANGELOG` не обязателен; SemVer вводится при появлении внешних пользователей.
...
### Уровень 1b: privacy-first транспорт (protocol profile)
...
| SPEC-425 | `spec/SPEC-425-updates-and-migrations.md`            |
